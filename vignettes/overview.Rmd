...
In order to be able to construct a popsnetwork object we have to provide an external input rate for
each source.
...
sg_net_dir <- spread_dirichlet(sg_net, 1.3, list(sg_sources, sg_freqs))
sg_net_dir
```

Now we are ready to draw isolates.
